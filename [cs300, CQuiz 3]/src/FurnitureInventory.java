 ...
 * 
 * Note: Do NOT add any additional data fields! You are allowed to add private helper methods if
 * needed. Note: MAKE SURE TO SAVE your source file before uploading it to gradescope.
 */
class Furniture implements Comparable<Furniture> {
  ...
  public Furniture(String name, int weight) {
    this.name = name;
    this.weight = weight;
    INVENTORY_ID = Furniture.inventoryNumGenerator; // CHANGE THIS, included only to avoid compile
                                                    // errors
    Furniture.inventoryNumGenerator++;
  }
  ...
  public boolean equals(Object o) {
    return (o instanceof Furniture) && (((Furniture) o).getName().equals(this.name));
  }

  // add any methods required by the Comparable interface here.
 ...
 * methods for analyzing the contents of this inventory. Note: Do NOT add any additional data
 * fields! You are allowed to add private helper methods if needed.
 */
public class FurnitureInventory {
  ...
  public static FurnitureInventory copyOf(FurnitureInventory toCopy) {
    ...
    FurnitureInventory c = new FurnitureInventory();
    for (int i = 0; i < toCopy.list.size(); i++)
      c.list.add(toCopy.list.get(i));
  ...
  public int getHeaviestFurniture() {
    ...
    // below.
    this.sortInventory();
    Furniture f = this.list.get(this.list.size() - 1);

    return f.getInventoryId();
  }

  public static void main(String[] args) {
    FurnitureInventory c = new FurnitureInventory();
    Furniture f1 = new Furniture(FurnitureInventory.names[0], 5);
    Furniture f3 = new Furniture(FurnitureInventory.names[0], 10);
    Furniture f2 = new Furniture(FurnitureInventory.names[1], 6);
    c.list.add(f1);
    c.list.add(f2);
    c.list.add(f3);

    FurnitureInventory c2 = FurnitureInventory.copyOf(c);
