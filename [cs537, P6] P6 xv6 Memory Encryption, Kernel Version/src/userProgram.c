...
int main(void) {
    ...
    // These pages are used to make sure the test result is consistent for
    // different text pages number
    ...
    //     pt_entries[i] = 0x0;
    // }
    // for (int i = 0; i < PGSIZE * sizeof(char); i++) buffer[i] = 0;

    uint text_pages = (uint)boundary / PGSIZE;
    if (text_pages > expected_dummy_pages_num - 1)
        err("XV6_TEST_OUTPUT: program size exceeds the maximum allowed size. "
            "Please let us know if this case happens\n");

    for (int i = 0; i < text_pages; i++) dummy_pages[i] = (char *)(i * PGSIZE);
    ...
    // Now we should have expected_dummy_pages_num + 1 (buffer) pages in the
    // clock queue Fill up the remainig slot with heap-allocated page and bring
    // all of them into the clock queue
    ...
    if (fork() == 0) {
        ...
        if (retval == 1) {
            for (int i = 0; i < retval; i++) {
                printf(1,
                       "XV6_TEST_OUTPUT Index %d: pdx: 0x%x, ptx: 0x%x, "
                       "writable bit: %d, encrypted: %d, ref: %d\n",
                       i, pt_entries[i].pdx, pt_entries[i].ptx,
                       pt_entries[i].writable, pt_entries[i].encrypted,
                       pt_entries[i].ref);

                uint expected = 0x00;
                if (pt_entries[i].encrypted) expected = ~0x00;
                ...
                for (int j = 0; j < PGSIZE; j++) {
                    if (buffer[j] != (char)expected) {
                        ...
                        printf(1,
                               "XV6_TEST_OUTPUT: content is incorrect at "
                               "address 0x%x: expected 0x%x, got 0x%x\n",
                               ((uint)(pt_entries[i].pdx) << 22 |
                                (pt_entries[i].ptx) << 12) +
                                   j,
                               expected & 0xFF, buffer[j] & 0xFF);
                        exit();
                    }
                }
            }
        ...
        if (retval == heap_pages_num + 1) {
            for (int i = 0; i < retval; i++) {
                ...
                uint expected = 0x0;
                if (pt_entries[i].encrypted) expected = ~0x0;
