//
// request.c: Does the bulk of the work for the web server, handling requests.
//

#include "helper.h"
#include "request.h"

// requestError(      fd,    filename,        "404",    "Not found", "CS537
...
void requestServeDynamic(int fd, char *filename, char *cgiargs) {
    ...
    if (Fork() == 0) {
        ...
        /* When the CGI process writes to stdout, it will instead go to the
         * socket */
