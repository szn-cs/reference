...
// Wrapper functions for the system calls invoked by the basic web server and
// client. Adding error checking error codes for system calls.
...
int Gethostname(char *name, size_t len) {
    ...
    if ((rc = gethostname(name, len)) < 0) unix_error("Setenv error");
    return rc;
}
...
int Setenv(const char *name, const char *value, int overwrite) {
    ...
    if ((rc = setenv(name, value, overwrite)) < 0) unix_error("Setenv error");
    return rc;
}
