 ...
 * address of allocated block on success (payload of 'size' bytes), NULL on failure. 
 */
 ...
 * - modifies a single header & footer in coalesced free block, without clearing up older data
 * 
...
void dispMem() {     
    ...
    while (current->size_status != 1) {
        ...
        if (t_size & 1) {
            ...
            strcpy(status, "Free");
            is_used = 0;
            blockHeader* footerBlock = (void*) current + getSize(current) - sizeof(blockHeader);
        ...
        p_status, (unsigned long int)t_begin, (unsigned long int)t_end, footer, t_size);
 ...
 * freeBlock: free block to split if possible and satisfies splinting requirement
 * SUCCESS on successful split of free block, otherwise FAILURE.
 */
int splitBlockPolicy(int *blockSize, blockHeader* freeBlock) {
...
/*
 * Iterator's next block with wrap around - get next block wrapping around the end of heap
 * 
 * b: current block
 ...
 * fullfilling requirement - total size of the allocated block must be a multiple of 8
 * 
